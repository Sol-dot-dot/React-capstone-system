...
const EnhancedBorrowedBooksScreen = ({ userData, onBack }) => {
  ...
  const getBookStatus = (dueDate) => {
    ...
    if (diffDays < 0) {
      ...
      return { status: 'due-soon', color: ModernTheme.colors.warning, text: 'Due Soon' };
    } else {
      return { status: 'active', color: ModernTheme.colors.success, text: 'Active' };
    }
  };
  ...
  const renderEmptyState = () => (
    ...
    >
      ...
      <Text style={styles.emptySubtitle}>
        You haven't borrowed any books yet. Visit the library to start borrowing!
      </Text>
    </Animatable.View>
  );
...
export default EnhancedBorrowedBooksScreen;
