    ...
    try {
        ...
        for (const user of users) {
            try {
                ...
                console.log(`Inserted user: ${user[0]}`);
            } catch (error) {
        ...
        for (const book of books) {
            try {
                ...
                if (error.code === 'ER_DUP_ENTRY') {
                    console.log(`Book ${book[0]} already exists, skipping...`);
                } else {
                    console.error(`Error inserting book ${book[0]}:`, error.message);
                }
            }
        }
        ...
        const transactions = [
            ['C22-0044', 3, '2025-09-01', '2025-09-15', 'borrowed'],
            ['C22-0045', 1, '2025-09-02', '2025-09-16', 'returned'],
            ['C22-0044', 2, '2025-09-03', '2025-09-17', 'borrowed']
        ];

        for (const transaction of transactions) {
            try {
                await pool.execute(
                    'INSERT INTO borrowing_transactions (student_id_number, book_id, borrow_date, due_date, status) VALUES (?, ?, ?, ?, ?)',
                    transaction
                );
                console.log(`Inserted transaction for student: ${transaction[0]}`);
            } catch (error) {
        ...
        for (const log of loginLogs) {
            try {
                ...
                console.log(`Inserted login log for user: ${log[0]}`);
            } catch (error) {
        ...
        // Insert sample fines
        const fines = [
            ['C22-0044', 1, 15.00, 'Overdue book return', 'unpaid', '2025-09-09 12:00:00'],
            ['C22-0045', 2, 10.00, 'Overdue book return', 'paid', '2025-09-08 14:30:00']
        ];

        for (const fine of fines) {
            try {
                await pool.execute(
                    'INSERT INTO fines (student_id_number, transaction_id, fine_amount, fine_reason, status, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                    fine
                );
                console.log(`Inserted fine for student: ${fine[0]}`);
            } catch (error) {
        ...
        for (const tracking of semesterTracking) {
            try {
                await pool.execute(
                    'INSERT INTO semester_tracking (student_id_number, semester, books_borrowed_count, status, created_at) VALUES (?, ?, ?, ?, ?)',
                    tracking
                );
                console.log(`Inserted semester tracking for student: ${tracking[0]}`);
            } catch (error) {
                console.error(`Error inserting semester tracking:`, error.message);
            }
        }
