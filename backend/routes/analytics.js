...
// System Analytics - Get system performance metrics
  ...
  try {
    ...
    const systemMetricsQuery = `
      SELECT 
        ...
        COUNT(CASE WHEN action = 'borrow' THEN 1 END) as borrows,
    ...
    // Execute queries
    const [systemMetrics] = await db.execute(systemMetricsQuery, [startDate]);
    const [errorRates] = await db.execute(errorRatesQuery, [startDate]);
    const [activityPatterns] = await db.execute(activityPatternsQuery, [startDate]);
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to fetch system analytics data',
      error: error.message
    });
  }
});
